...
exports.extractData=function(fileName){
  let workbook_nanp = new Excel.Workbook();
  const promise1 = workbook_nanp.xlsx.readFile(fileName)
    .then(() =>{
      var data=[];
      let worksheet = workbook_nanp.getWorksheet(1);
...
exports.pushRegion=function(fileName,regions){
let workbook_nanp = new Excel.Workbook();
    let workbook_numbers = new Excel.Workbook();
    const promise1 = workbook_nanp.xlsx.readFile(fileName)
      .then(function () {
        let w = workbook_nanp.getWorksheet(1);
        
          let x=workbook_numbers.addWorksheet("abc");
          let index=0;
          w.eachRow(function (row,rowNumber) {
            var values=row.values;
            if (rowNumber !== 1) {
              values.push(regions[index]);
              index++;
              x.addRow(values);
            } else {
              values.push("region");
          ...
          workbook_numbers.xlsx.writeFile(fileName).then(()=>{
        });
            });
    });
    return promise1;
